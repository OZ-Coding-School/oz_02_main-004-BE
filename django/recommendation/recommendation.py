...
def get_recommendations(user_id, num_recommendations=5):
    ...
    interactions = UserToDoInteraction.objects.all().values(
        "user_id", "todo_id", "interaction_type"
    )
    df = pd.DataFrame(list(interactions))

    # 사용자-아이템 행렬 생성
    user_item_matrix = df.pivot_table(
        index="user_id", columns="todo_id", aggfunc="size", fill_value=0
    )
    ...
    recommendations = similar_users_items[
        ~similar_users_items.index.isin(user_interactions)
    ]

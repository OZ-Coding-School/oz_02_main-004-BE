...
class Pet(CommonModel):
    ...
    hunger_degree = models.DateTimeField(
        null=True, blank=True, verbose_name="최근 식사 시간"
    )  # 최근 밥이나 간식 준 시간
    ...
    active_pet = models.ForeignKey(
        PetCollection,
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name="active_pet_pets",
        verbose_name="키우는 펫 아이템",
    )
    streak = models.PositiveIntegerField(default=0)
    last_snack_date = models.DateField(
        null=True, blank=True, verbose_name="최근 간식(밥) 날짜"
    )
